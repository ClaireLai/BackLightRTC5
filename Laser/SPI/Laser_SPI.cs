...
namespace BackLight.Laser.SPI
{
    class Laser_SPI:ILaser
    {
        ///////////////////////////////////////////////
        ///                 Para                    ///
        ///////////////////////////////////////////////
        ...
        public override RetErr Close()
        {
            ...
            try
            {
                if (gPort == null) throw new Exception("Laser Port not init");
                if (gPort.IsOpen)
                    gPort.Close();
            }
            catch (Exception ee)
        ...
        public override RetErr OpenShutterAndDIODE(bool POpen)
        {
            ...
            try
            {
                if (gInfo.IntFacMode != 1) return ret;
                string cmd = "GS\r\n";
                bool rt = Cmd(cmd, out string rec) ? true : throw new Exception("Cmd fail:" + cmd);
                if (rec.Contains("E"))
                    throw new Exception("Cmd fail:" + cmd);
...
namespace BackLight.Laser.SPI.Define
{
    struct Info
    {
        public int Port;
